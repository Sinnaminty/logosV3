...
- **Use `utils::reply_ok` / `reply_info`** instead of constructing `Reply`/`EmbedType` manually. Command files should not import `EmbedType`. Errors flow through Poise's `on_error` hook in `handlers.rs`, not through a `reply_err` helper (none exists).
